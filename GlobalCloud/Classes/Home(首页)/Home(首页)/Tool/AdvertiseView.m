...
- (instancetype)initWithTitles:(NSArray *)titles{
    ...
    if (self) {
        ...
        self.color = UIColorFromRGBA(0x999999, 1.0);
        self.time = 3.5f;
...
- (void)layoutSubviews{
    ...
    if (self.headImg) {
      ...
      //  self.headImageView.center.y = center.y;
        
//        // 1. 用一个临时变量保存返回值。
        CGRect frame =  CGRectMake(self.edgeInsets.left, self.edgeInsets.top, kNoticeImageViewWidth, kNoticeImageViewWidth);
        margin = CGRectGetMaxX(self.headImageView.frame) +10;

        frame.origin.y = (self.frame.size.height-kNoticeImageViewWidth)/2;
//        // 3. 修改frame的值
