...
-(void)setUpPageControl:(NSArray *)array
{
   ...
   // NSLog(@"scrollView-frame:%f",self.scrollView.contentOffset.x);
    ...
    pageControl.currentPageIndicatorTintColor = UIColorFromRGBA(0xF3650D, 1.0);
    //[UIColor orangeColor];ffa600
...
STPageControl *pageControl = [STPageControl pageControlDefaultWithFrame:CGRectMake(100, self.frame.size.height - 20, 0, 0)
                                                              numberOfPages:array.count
                                                            coreNormalColor:[UIColor whiteColor]
                                                          coreSelectedColor:UIColorFromRGBA(0x20CEC2, 1.0)
                                                            lineNormalColor:[UIColor whiteColor]
                                                          lineSelectedColor:UIColorFromRGBA(0x20CEC2, 1.0)];
     [pageControl addTarget:self action:@selector(pageChange:) forControlEvents:UIControlEventValueChanged];
     pageControl.center = CGPointMake(self.center.x, self.frame.size.height - 20);
     self.pageControl = pageControl;
...
{
    if (!_duration) {
        self.timer = [NSTimer timerWithTimeInterval:4.5 target:self selector:@selector(updateTimer) userInfo:nil repeats:YES];
    }else{
...
-(void)updateTimer
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.scrollView.contentOffset.x > self.frame.size.width * self.imageArray.count) {
            [self.scrollView setContentOffset:CGPointMake(self.frame.size.width, 0) animated:NO];
            newOffset = CGPointMake(self.frame.size.width * 2, 0);
        }else {
            newOffset = CGPointMake(self.scrollView.contentOffset.x  + CGRectGetWidth(self.scrollView.frame), 0);
        }
...
{
    NSLog(@"点击了广告第%ld页",self.pageControl.currentPage);
