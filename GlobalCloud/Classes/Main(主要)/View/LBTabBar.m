...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self=[super initWithFrame:frame]) {
        ...
        [self addAnimationView:plusBtn];
        
    }
    return self;
}

-(void)addAnimationView:(UIButton *)sender{

    UIView *redView = [[UIView alloc]init];
    self.redView = redView;
    redView.centerX = self.plusBtn.width/2;
    //调整发布按钮的中线点Y值
    redView.centerY = self.plusBtn.height/2+3 ;
    redView.bounds = CGRectMake(0, 0, 30, 30);
    
    UILabel *label = [[UILabel alloc] init];
    label.frame = redView.bounds;
    label .text = @"赚";
    ...
    redView.cornerRadius = 15;
    redView.clipsToBounds = YES;
    [self.plusBtn addSubview:redView];
    [self startAnimation:redView];
}


- (void)startAnimation:(UIView *)redview
{
